...
- [ ] **Task 2.4**: Implement session termination (tmux kill-session)
  - **Files:** `src/server/SessionManager.ts` (modify)
  - **Rationale:** Allow dashboard to terminate sessions (with confirmation in UI)
  - **Dependencies:** Task 2.2
...
- [ ] **Task 6.4**: Add session kill button with confirmation dialog
  ...
  - **Rationale:** Kill button on card with confirmation dialog to prevent accidents
...
- [ ] Can discover existing `agentboard-*` tmux sessions
- [ ] Can create new session and see it appear
- [ ] Can kill session with confirmation dialog
