...
- **~~node-pty compilation issues~~** (severity: ~~medium~~ **eliminated**)
  ...
  - **Fallback alternatives if Bun PTY has issues:**
    ...
    - `tmux pipe-pane` + `send-keys` (no PTY, uses tmux native I/O - simpler but reduced interactivity)
    - `tmux capture-pane` polling (simpler but less responsive)

- **Bun not installed** (severity: low)
  - Users may not have Bun installed
  - **Mitigation:** Document Bun installation in README (`curl -fsSL https://bun.sh/install | bash`). Package.json scripts can fall back to npm/node if needed.
