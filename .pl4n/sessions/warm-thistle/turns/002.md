...
- **Answer:** Use Option A (separate sessions) for MVP. Each project gets its own `agentboard-{project}` tmux session. The "ingest external sessions" feature (discovering and monitoring non-agentboard Claude sessions) can be added as a post-MVP enhancement via `DISCOVER_PREFIXES` env var. why bother with separate sessions, does that buy us anything? note we may want multipel active windows per project anyway
...
for tyepscript, maybe sure we use bun oxlint etc, the fastest/best in class for testing linting building
...
make sure we also have handling for scrolling the tmux windows with tap + drag or mouse wheel, etc. and copy+paste in and out
...
- **node-pty compilation issues** (severity: medium) do we want to be usign node-pty? what are the alts
...
- **tmux not installed or inaccessible** (severity: high) shoudl install as part of the setup or something
