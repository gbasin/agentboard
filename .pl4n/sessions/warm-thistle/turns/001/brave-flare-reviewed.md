...
| State machine | Simple switch (XState optional) |
...
**Q4: Should session discovery be limited to `claude-*` prefix, or allow configurable prefix?**
- Context: Users may have different naming conventions for their tmux sessions
- Agent recommends: Start with `claude-*` hardcoded; add `SESSION_PREFIX` env var later if requested. Keep MVP simple.
- **Answer:** _(pending)_

**Q5: Should the UI expose a "Kill session" action in MVP?**
- Context: Kill is useful but risky without authentication. Anyone on the network could kill sessions.
- Agent recommends: **Omit kill button from UI in MVP**. Keep backend capability but don't expose in UI until auth is added. Users can kill via tmux directly.
- **Answer:** _(pending)_

**Q6: Notification behavior when dashboard tab is focused?**
- Context: Avoid noisy alerts during active use
- Agent recommends: Sound + favicon badge always; browser Notification API only when tab is hidden. Debounce repeated status changes.
- **Answer:** _(pending)_
...
- **Log mapping mismatch** (severity: **high**)
  ...
  - **Mitigation:** Verified escaping format by examining `~/.claude/projects/`. Test against real logs. Fall back to 'unknown' status gracefully.
