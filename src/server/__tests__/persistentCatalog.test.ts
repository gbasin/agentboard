...
function setup() {
  const db = initDatabase({path:':memory:'}); databases.push(db)
  return {db,catalog:new SessionCatalog(db.db,'host-test')}
}
afterEach(() => { for (const db of databases.splice(0)) db.close() })
const input = {name:'A-Runtime',projectPath:'/project',command:'sh',lastActivityAt:'2020-01-01T00:00:00.000Z'}

describe('durable catalog', () => {
  test('saves a session before any provider log or process exists', () => {
    const {db,catalog} = setup(); const created = catalog.create(input)
  ...
  test('keeps old names searchable and archives without deleting', () => {
    const {catalog} = setup(); const saved = catalog.create(input)
    catalog.rename(saved.id,'A-Updated'); catalog.transition(saved.id,'archived'); catalog.pin(saved.id,true)
    expect(catalog.history({q:'A-Runtime',pinned:true}).sessions[0]?.name).toBe('A-Updated')
  ...
  test('launch requests are idempotent and concurrent requests cannot create two open runs', () => {
    ...
    expect(catalog.beginRun(saved.id,'request-1')).toEqual({id:run.id,reused:true})
    expect(()=>catalog.beginRun(saved.id,'request-2')).toThrow('already running or starting')
    ...
    expect(()=>catalog.bind(saved.id,run.id,'ab:@1','epoch1')).toThrow('no longer current')
  })
  test('history pages cover all equal-time records exactly once', () => {
    ...
    do { const page=catalog.history({limit:2,cursor});ids.push(...page.sessions.map(s=>s.id));cursor=page.nextCursor || undefined } while(cursor)
    expect(ids).toHaveLength(7);expect(new Set(ids).size).toBe(7)
    expect(()=>catalog.history({cursor:'invalid'})).toThrow('Invalid history cursor')
    expect(catalog.history({q:'%'}).sessions).toHaveLength(0)
  })
  test('keeps workspace membership across catalog reopen', () => {
    const {catalog,db} = setup();const saved=catalog.create(input)
    const workspace=catalog.saveWorkspace('Team A',[saved.id,saved.id])
    expect(new SessionCatalog(db.db,'host-test').workspaces()[0]).toEqual({...workspace,sessionIds:[saved.id]})
    expect(()=>catalog.saveWorkspace('Bad',['absent'])).toThrow('missing session')
  })
  test('merges a discovery import into its managed session without losing workspace membership', () => {
    const {catalog} = setup()
    const imported = catalog.create({...input, name:'Imported', providerId:'provider', origin:'imported', pinned:true})
    ...
    expect(catalog.workspaces().find(w => w.id === workspace.id)?.sessionIds).toEqual([managed.id])
  })
})
...
describe('recovery reconciliation', () => {
  function fixture() {
    const {db}=setup()
    let snapshot={epoch:'epoch1',windows:new Map<string,{boardId:string;runId:string}>()}
    const calls:string[]=[]
    const manager={setWindowOption(){},renameWindow(){},killWindow(w:string){calls.push(w)}} as unknown as SessionManager
    const persistence=new PersistentSessions(db,manager,'host-test',()=>snapshot)
    const saved=persistence.catalog.create(input);const run=persistence.catalog.beginRun(saved.id)
    persistence.catalog.bind(saved.id,run.id,'ab:@1','epoch1')
    snapshot.windows.set('ab:@1',{boardId:saved.id,runId:run.id})
    const live:Session={id:'ab:@1',name:input.name,tmuxWindow:'ab:@1',projectPath:'/project',status:'unknown',createdAt:'2020-01-01',lastActivity:'2020-01-01',source:'managed'}
    return {persistence,saved,run,live,calls,setSnapshot:(value:typeof snapshot)=>{snapshot=value}}
  }
  test('a reliable empty replacement server interrupts sessions without killing windows', () => {
    ...
    setSnapshot({epoch:'epoch2',windows:new Map()});persistence.beforeSnapshot([])
    expect(persistence.catalog.get(saved.id)?.state).toBe('interrupted');expect(calls).toEqual([])
  })
  test('a transient empty worker result does not erase a live session', () => {
    const {persistence,saved}=fixture();persistence.beforeSnapshot([])
  ...
  test('reused window IDs do not claim or kill another run', () => {
    ...
    setSnapshot({epoch:'epoch2',windows:new Map([['ab:@1',{boardId:'another',runId:'other'}]])})
  ...
  test('adopts a tagged pane created before the database binding was saved', () => {
    ...
    persistence.catalog.transition(saved.id,'interrupted');const run=persistence.catalog.beginRun(saved.id)
    setSnapshot({epoch:'epoch2',windows:new Map([['ab:@1',{boardId:saved.id,runId:run.id}]])})
    persistence.beforeSnapshot([live])
    expect(persistence.catalog.get(saved.id)?.state).toBe('running')
  ...
  test('finishes a persisted hibernation intent after a backend crash', () => {
    ...
    persistence.db.db.query('UPDATE board_sessions SET requested_state=? WHERE id=?').run('hibernating',saved.id)
    const sessions=[live]
