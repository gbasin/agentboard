...
export function createCatalogSchema(db: Database) {
  ...
  db.transaction(() => {
    ...
    CREATE TABLE IF NOT EXISTS board_sessions (
      ...
      state TEXT NOT NULL CHECK(state IN ('starting','running','interrupted','hibernating','archived','failed')),
      pinned INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL,
      last_activity_at TEXT NOT NULL, current_window TEXT, epoch TEXT,
      error TEXT, preview TEXT,
      origin TEXT NOT NULL CHECK(origin IN ('managed','discovered','imported')),
      last_run_id TEXT,
      requested_state TEXT CHECK(requested_state IN ('hibernating','archived')),
      terminal_preview TEXT, terminal_preview_at TEXT
    );
