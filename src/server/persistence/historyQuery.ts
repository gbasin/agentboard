...
import type { HistoryPage, HistoryQuery, SavedSession } from '../../shared/persistence'
...
export function queryHistory(db: Database, query: HistoryQuery = {}): HistoryPage {
  const where: string[] = []
  const values: SQLQueryBindings[] = []
  const add = (sql: string, value?: SQLQueryBindings) => { where.push(sql); if (value !== undefined) values.push(value) }
  if (query.q?.trim()) {
  ...
  if (query.state === 'previously-open') add("origin != 'imported' OR EXISTS (SELECT 1 FROM session_runs r WHERE r.session_id=board_sessions.id)")
  ...
  if (query.hours && query.hours > 0) add('last_activity_at>=?', new Date(Date.now() - query.hours * 3600000).toISOString())
  if (query.cursor) {
    try {
      const cursor = JSON.parse(Buffer.from(query.cursor, 'base64url').toString())
      if (!Array.isArray(cursor) || cursor.length !== 2 || cursor.some(x => typeof x !== 'string')) throw Error()
  ...
  const rows = db.query(`${sessionSelect} ${where.length ? `WHERE ${where.map(w => `(${w})`).join(' AND ')}` : ''}
    ORDER BY last_activity_at DESC, id DESC LIMIT ?`).all(...values, limit + 1).map(r => readSession(r)!)
  const sessions = rows.slice(0, limit)
  const last = sessions.at(-1)
  const nextCursor = rows.length > limit && last
    ? Buffer.from(JSON.stringify([last.lastActivityAt, last.id])).toString('base64url') : null
